...
best possible way to reproducing Axelrod's work which is the subject of the next
section.
...
Although, interestingly the environment does not allow the high performing
...
\subsection{Tournament with extortion}\label{sec:run_with_stewart_plotkin}
...
Determinant strategies from~\cite{Stewart2012}. The change of rank across the
tournaments is shown in Figure~\ref{fig:rank_change_in_sp_tournament}. The tournament 
exhibits limited relative change in performance, with the zero-determinant strategies 
...
take advantage of noise (the DBS strategy) win.
...
best practices code that was written in the 1980s. The archive of this work
...
at~\url{https://github.com/Axelrod-Python/axelrod-fortran}.

Using this, we have been able to run the original, or as close to original
versions of Axelrod's seminal tournament in the field of emergence of
cooperation. The results match well except for a single strategy by Champion.
Because of the approach taken we are confident in the results presented here as
this particular strategy has also been translated to the modern Axelrod Python
library.
...
highly context-dependent and do not generalize well to broader settings.
...
Moreover, when we attempt to approximate the full strategy space using the
...
understand and reproducibility of the work. Expanding this process in the open
