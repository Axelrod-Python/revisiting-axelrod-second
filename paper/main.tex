...
the analysis in several directions by incorporating both classical strategies
from the repeated games literature and additional strategies from the \AXL{}
library, ultimately conducting one of the largest \IPD{} tournament studies to
date. These extensions shed new light on the robustness and context dependence
...
the original tournament. We consider strategies such as Cooperator, Defector,
Win--Stay--Lose--Shift, and \GTFT, among others. We also include
more recent strategies, such as All-or-nothing-2 (\AON) and EvolvedLookerUp2 2
2~\cite{Knight2017}.
...
\TFT{}'s rank and the winner of the tournament. In this section, we use
...
Notably, several benchmark strategies, including Win--Stay--Lose--Shift and
\AON{} (a delayed version of Win--Stay--Lose--Shift), do not
outperform \TFT{}. Even the strategy EvolvedLookerUp2 2 2 capable of exploiting
weaker opponents fails to surpass it. \GTFT{} ranks third overall
but this remains insufficient to dethrone \TFT{}.
We find that the only strategy capable of dethroning \TFT{} is Anti Tit For Tat.
This strategy begins with cooperation and subsequently plays the opposite of its
opponent's previous move. Although simple and seemingly counterintuitive, in
repeated games with a finite expected number of rounds it can achieve a higher
payoff than \TFT{}. Interestingly, the more complex strategy \texttt{k42r} is
able to exploit Anti Tit For Tat, achieving an average score of 3.5337 and
ultimately rising to the top of the tournament.

However, this advantage does not persist when all nine additional strategies
are introduced simultaneously. Under these conditions, \TFT{} is able to
benefit from interactions with a broader and more cooperative set of
opponents, weakening the disadvantage introduced by Anti Tit For Tat.
...
The \AXL{} library includes
\input{./assets/number_of_strategies_in_full_tournament.tex} strategies that did
not appear in the original tournament. As before, we begin with the original
tournament consisting of the 63 submitted strategies and examine what happens
when additional strategies are introduced. For a given subset size, we consider
every possible combination of strategies drawn from the pool of all additional
strategies and add them to the original tournament. 
...
As in the classical strategy analysis, the probability that \TFT{} wins does
not simply decrease as more strategies are introduced. Instead, the winning
probability fluctuates with subset size: adding a single additional strategy
substantially increases the probability that \TFT{} wins (to about 85\%); with
two invitees the probability is lower (\(\sim\)71\%), with four invitees it
increases again (\(\sim\)75\%), and only with three invitees do we observe a
larger reduction (to \(\sim\)60\%).

The previous analysis with classical strategies demonstrated that such
non-monotonic behaviour can emerge from the combinatorial structure of the
introduced subsets, where specific strategies disproportionately alter the
tournament dynamics. In the present setting, however, the number of possible
tournaments becomes substantially larger, making it considerably more difficult
to identify the precise structural causes of these fluctuations.

Nevertheless, these results further highlight the robustness of \TFT{} in
Axelrod's second tournament: it remains difficult to unseat as the top
...
\TFT{}~\cite{nowak1992tit} (\GTFT), and ``All-or-nothing''~\cite{hilbe2017memory}
...
More broadly, our results raise questions about the extent to which tournament
rankings can be considered ``objective''. In repeated game tournaments,
strategies are not evaluated in isolation, but relative to the interaction
environment generated by the participating strategies. As demonstrated by our
extra invitation experiments, modifying the tournament composition can
substantially alter both rankings and cooperation dynamics.

In the \SI, we examine the distributions of cooperation
rates and payoffs across pairwise interactions for both the original tournament
and the expanded \AXL{} tournament. In both cases, these distributions are
highly skewed, indicating that the tournaments do not sample interactions
uniformly across behavioural space. This raises a broader open question: what
would constitute an ``objective'' tournament environment for evaluating
strategies? Should tournaments aim to sample interactions uniformly across
behavioural space, or balance different forms of reciprocity, exploitation, and
cooperation? Our results suggest that tournament rankings are inherently linked
to the interaction structures induced by the participating strategies, rather
than representing absolute measures of strategic quality.
