...
\newcommand{\GTFT}{\emph{GTFT}}
\newcommand{\AON}{\emph{AON2}}
...
Axelrod's second tournament included 63 strategies. While Axelrod named a few
...
names is given in the \SI~(S1). Notably, the original tournament did not include
...
Cooperate), Defector (Always Defect), or Win--Stay--Lose--Shift~\cite{nowak1993strategy}.
...
In \SI{} (\textbf{S8}), we consider the original set of players with a truly
...
\textbf{d,} Violin plots showing the distribution of mean scores for the top 15
strategies across all repetitions (n = 25{,}000 independent tournament
...
behaviour, is provided in in the \SI~(S5, Figure 1). In summary, the strategy has two states:
...
\texttt{k42r} ranked only 3rd, suggesting that these mechanisms were not
...
We investigate how the tournament outcomes would change if Axelrod had received
additional submissions. To this end, we begin by asking what would have happened
had several well-known strategies from the \IPD{} literature been submitted to
the original tournament. We consider classical strategies such as Cooperator, Defector,
Win--Stay--Lose--Shift, and Generous \TFT{}~\cite{nowak1992tit} (\GTFT), among
others. We also include more recent strategies, such as
``All-or-nothing-2''~\cite{hilbe2017memory} (\AON) and EvolvedLookerUp2 2
2~\cite{Knight2017}.

We rerun the tournament using the original set of strategies while adding each
...
efficiency, each tournament is repeated 250 times.

Several benchmark strategies, including Win--Stay--Lose--Shift and
...
We find that the only strategy capable of changing the outcome of the
...
In particular, the more complex strategy \texttt{k42r} exploits Anti Tit For
...
opportunities for exploitative strategies such as \texttt{k42r} to achieve
exceptionally high scores.

This effect does not persist when all nine additional strategies are introduced
...
\begin{table}[!hbtp]
    ...
    \caption{\textbf{Effect of adding classical strategies to Axelrod's second tournament.}
    We examine how the results change when a single strategy from a set of
...
We next extend this analysis by considering combinations of classical
...
the performance of \texttt{k42r} to provide a broader comparison of how the
additional strategies influence tournament dynamics.

In Table~\ref{tbl:classical_strategies_combinations}, we show the winning
probabilities of \TFT{} and \texttt{k42r} as a function of the subset size.
...
\TFT{} ranks second while \texttt{k42r} ranks first. As a result, the winning
...
In Figure~\ref{fig:classical_strategies_combinations}\textbf{a}, we show the
distribution of the average payoffs achieved by \TFT{} and \texttt{k42r} across
...
by \texttt{k42r} increases for intermediate subset sizes, the average payoff of
\TFT{} remains consistently higher, indicating that \TFT{} still performs
better overall on average.
In Figure~\ref{fig:classical_strategies_combinations}\textbf{b}, we additionally
...
\begin{table}[!hbtp]
    ...
    \caption{\textbf{Winning probabilities of \TFT{} and \texttt{k42r} with
    different combinations of classical strategies.} We consider all possible
    ...
    and \texttt{k42r}. The number of combinations for each subset size is
    denoted by \(N\).}
...
\begin{figure}[!hbtp]
    \centering
    \includegraphics[width=\textwidth]{./assets/extra_invitations_classical_strategies.pdf}
    \caption{\textbf{Effect of additional classical strategies on average payoffs and tournament cooperation.}
    \textbf{a,} Distributions of the average tournament payoffs achieved by
    \TFT{} (\texttt{k92r}) and \texttt{k42r} as a function of the number of added classical
    ...
    for each subset size. \textbf{b,} Distribution of the average cooperation
    ...
    under which \TFT{} performs least effectively relative to \texttt{k42r}. All
...
The previous analysis focused on a small set of well-known classical strategies,
...
The \AXL{} library includes 209 strategies that did not appear in the original
...
strategies drawn from the 209 additional \AXL{} strategies and add them to the
original tournament.

Due to the combinatorial growth in the number of possible tournaments, we limit
...
A striking pattern is that the effect of additional invitees on \TFT{} remains
...
are introduced. Instead, it fluctuates with subset size: adding a single
additional strategy increases the probability that \TFT{} wins to
approximately \(85\%\); with two invitees the probability decreases to
...
The previous analysis with classical strategies demonstrated that such
...
opportunities for more exploitative strategies such as \texttt{k42r}. This
...
In the present setting, however, the number of possible tournaments and
...
Nevertheless, the results further highlight the robustness of \TFT{} in
Axelrod's second tournament: despite the introduction of large numbers of
additional strategies, \TFT{} remains difficult to unseat as the top
performer. The next most successful strategy, \texttt{k42r}, remains competitive
(e.g., winning up to \(\sim\)37\% of tournaments with three additional
invitees), whereas strategies such as \texttt{k32r} and \texttt{k75r}, which
can achieve high mean scores in some realizations, tend to become less
...
\begin{table}[!hbtp]
        ...
        full list is provided in \SI{} 4 and 6, and source code/documentation are
...
Win--Stay--Lose--Shift, \GTFT, and ``All-or-nothing''
...
More broadly, our results highlight that tournament rankings in repeated games
...
This also raises a broader question regarding the environments in which these
...
scripts, has been archived alongside the data in the Zenodo
repository~\cite{knight_2025_17250038} at
\url{https://doi.org/10.5281/zenodo.17250038}, which provides a permanent and
citable reference to the exact version used in this study. The code is also
accessible via the project's online repository:
\url{https://github.com/Axelrod-Python/revisiting-axelrod-second/}, and is
released under the MIT License, an Open Source Initiative approved licence.
...
\section*{Funding}
This work was not supported by any funding grants.
...
\begin{figure}[!hbtp]
    ...
    The panels follow the same layout as Figure~\ref{fig:reproduced_with_noise}.
    ...
    value is reported.
    \textbf{d,} Pairwise cooperation rates between strategies. For each pair we
